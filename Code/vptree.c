...
static node *node_create(vptree *vp, node *parent, int n, distp *dp, int *alli, int alln, void *user_data, void (*callback)(void *user_data, int i, int n), bool vsel);
...
static node *node_create(vptree *vp, node *parent, int n, distp *dp, int *alli, int alln, void *user_data, void (*callback)(void *user_data, int i, int n), bool vsel)
{
  ...
  nd =  static_cast<node*> (allocate(vp, sizeof(node)));
  if(nd == NULL) {
    return NULL;
  }
  nd->vp = vp;
  ...
  // Select reference node
    ...
    if( vsel == true){
        v = rand() % n;
        const void *pp = dp[v].p;
         //printf(dp[v]);
        int fi;
        for (fi = 0; fi < n; fi++){
            dp[fi].d = distance(vp, dp[fi].p,pp);
        }
        sort_distp(n, dp);
...
static int node_add(vptree *vp, node *nd, int n, distp *dp, int *alli, int alln, void *user_data, void (*callback)(void *user_data, int i, int n))
{
  ...
  for(i = 0; i < n; i++) {
    ...
    //printf("Calcualted distance for %d variables\n", i);
      //fprintf(stderr, "Sorting...\n");
  sort_distp(n, dp);
    
    
    double smdist = INFINITY;
    int mavidx;
    int fli;
    ...
    avdist =  avdist/n;
    for(fli = 0; fli < n; fli++){
        double adifd = avdist- dp[fli].d;
        adifd = sqrt(adifd*adifd);
        if(smdist >= adifd ){
            smdist = adifd ;
            mavidx = fli;
        }
    ...
    //for(fli = 0; fli < n; fli++){
    //    printf("Calcualted distance for %d variables: %f\n", fli, dp[fli].d);
    //}
    //printf("Total average distance: %f\n", avdist);
    //printf("Average idx %d and distance %f: \n", mavidx, dp[mavidx].d);
  ...
  //if(nd->mu < 0) {
    ...
    //else {
      //nd->mu = dp[m].d;
    //}
     //printf("Median distance for %d variables: %f\n", mavidx, nd->mu);
    nd->maxr = dp[n-1].d;
      //printf("Max distance for %d variables: %f\n", n-1, nd->maxr);
      
      int q2 = m+((n - m)/2);
    if(dp[q2].d >= nd->mu){
        nd->ftau = dp[q2].d - nd->mu;
    }
    else{
        nd->ftau = nd->mu - dp[q2].d;
    }


    //printf("Tau distance for %d variables: %f\n", q2, nd->ftau);
    
  //}

    
    
    //Comended this to check
  ...
  if(m > 0) {
    if(nd->lt == NULL) {
      nd->lt = node_create(vp, nd, m, lt, alli, alln, user_data, callback, false);
      if(nd->lt == NULL) {
        return -1;
      }
    }
  ...
  if(n - m > 0) {
    if(nd->ge == NULL) {
      nd->ge = node_create(vp, nd, n - m, ge, alli, alln, user_data, callback, false);
      if(nd->ge == NULL) {
        return -1;
      }
    }
  ...
  void *user_data, void (*callback)(void *user_data, int i, int n))
{
  ...
  dp = static_cast<distp*>(allocate(vp, n * sizeof(distp)));
  for(i = 0; i < n; i++) {
    dp[i].p = p[i];
  }
  ...
  // Add to tree
  if(vp->root == NULL) {
    //vp->root = node_create(vp, NULL, n, dp, &alli, n, user_data, callback, bool true);
      vp->root = node_create(vp, NULL, n, dp, &alli, n, user_data, callback, true);
    if(vp->root == NULL) {
      stat = -1;
    }
  }
...
static void nn_query(
                     ...
                     const void *p, int k,
                     const void **nn, double *nndist, int *sc)
{
  /*  double d, mu, ftau, maxr;
    int i, j;
    ...
    //if(d - nndist[k-1] < mu) {
    if(d - nndist[0] < mu) {
        nn_query(vp, nd->lt, p, k, nn, nndist, sc);
    }
    //if(d + nndist[k-1] >= mu) {
    if(d + nndist[0] >= mu) {
        nn_query(vp, nd->ge, p, k, nn, nndist, sc);
    }*/
}


static void nn_query2(
                     const vptree *vp, node *nd,
                     const void *p, int k,
                     const void **nn, double *nndist, int *sc, int *absc)
{
    double d, mu, ftau, maxr;
    int i, j;
    ...
    if(d <= maxr+ftau){
        if(d - ftau < mu) {
            nn_query2(vp, nd->lt, p, k, nn, nndist, sc, absc);
        }
    //if(d + nndist[k-1] >= mu) {
        if(d + ftau >= mu) {
            nn_query2(vp, nd->ge, p, k, nn, nndist, sc, absc);
        }
    }
    else{
        (*absc)++;
...
void vptree_nearest_neighbor(
                             const vptree *vp, const void *p,
                             int k, const void **nn, int *sc, int *absc)
{
    int i;
    double *nndist;
    ...
    // Call real algorithm
    //nn_query(vp, vp->root, p, k, nn, nndist, sc);
    //nn_query2(vp, vp->root, p, k, nn, nndist, sc);
    nn_query2(vp, vp->root, p, k, nn, nndist, sc, absc);
...
static incnode *make_incnode(const vptree *vp, incnode *parent, node *n, const void *q)
{
  ...
  incn = static_cast<incnode*>(allocate(vp, sizeof(incnode)));
  incn->n = n;
	...
	int k, const void **nn, int max_nodes)
{
  ...
  for(visited = 0; visited < max_nodes; visited++) {
    //pnd = pqueue_pop(pq);
    pnd = static_cast<pqnode*>(pqueue_pop(pq));
    if(pnd == NULL) {
      break;
    }
    nd = pnd->nd;
